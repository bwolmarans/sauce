...
while {0} {
   foreach {type val attr etype} [::XML::NextToken] break
   puts "Token:$token_counter Type:$type Value:$val Attribute:$attr Etype:$etype"
   if { $val == "title" } {
	   set title [lindex [::XML::NextToken] 1]
...
proc find_token { find_this_token {occurance 1} } {
	set token_counter 1
	while {1} {
	   foreach {type val attr etype} [::XML::NextToken] break
	   puts "Token:$token_counter Type:$type Value:$val Attribute:$attr Etype:$etype"
	   set token [list $type $val $attr $etype]
	   if { $val == $find_this_token } {
		   incr found_counter
       }
		   set token2 [::XML::NextToken]
		   #set token2 [::XML::NextToken]
		   return $token2
	   }
	   incr token_counter
...
set token_to_find "name"
puts "I found [find_token $token_to_find]"